...
int main(int argc, char **argv)
{
  ...
  nArray=_nomGetMethodList(aObject, FALSE, NULL);
  g_message("Num methods: %d", NOMArray_length(nArray, NULL)); 

  for(a=0; a < NOMArray_length(nArray, NULL) ; a++)
  {
    typedef void* NOMLINK nomProc(void*, void*);

    nomProc* nProc=_queryMethodToken(NOMArray_queryObjectAtIdx(nArray, a, NULL), NULL);
    
    g_message(" Method %s %x", _queryString(_getName(NOMArray_queryObjectAtIdx(nArray, a, NULL), NULL), NULL),
              _queryMethodToken(NOMArray_queryObjectAtIdx(nArray, a, NULL), NULL));  
    if(a==0)
      nProc(aObject, NULL);
  }
