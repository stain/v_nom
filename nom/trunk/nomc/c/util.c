...
/**
 Helper function which scans the current interface and the array of parent interfaces and returns
 the info structure of the given method. 
 
 \Param pif Pointer to an interface which is the start of the search. 
 \PARAM chrName Name of the method.
...
{
  int a;
  GPtrArray *pArray=pif->pMethodArray;
  gboolean fFound;
  PMETHOD pm;
  ...
  if(fFound)
    ...
    return findMethodInfoFromMethodName(pif, chrName);
}
